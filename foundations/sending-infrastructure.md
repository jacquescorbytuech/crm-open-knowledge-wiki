...
* **Reputation fragments and has to be managed deliberately.** Reputation is per sending domain and per IP, which is why a large programme runs pools, separating transactional from marketing and high-engagement from reactivation, and warms and monitors each independently. A single shared reputation that was fine at low volume becomes a liability when one careless segment can drag the whole estate toward spam.
