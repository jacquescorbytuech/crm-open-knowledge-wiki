...
* **Authenticate the payload.** Your endpoint is public, so verify each request is genuinely from the provider before acting on it, normally by checking an **HMAC signature** computed over the body with a shared secret. An unauthenticated webhook endpoint is a way to corrupt your own suppression list and metrics.
