...
Lead scoring ranks contacts so the programme can act on the ones worth acting on. Durable models score fit and engagement, keeping the two apart:
...
* **Marketing-qualified lead (MQL).** A contact whose combined fit and engagement clears the threshold marketing and sales have agreed means *ready for a sales conversation*. The threshold is a decision, not a fact: set it where the leads sales actually accepts cluster, then move it when acceptance drifts.
...
The handoff between them is where most B2B programmes leak. The fix is an explicit service-level agreement between the two teams. The SLA pins down the MQL and SQL definitions so both sides agree what qualifies. It commits sales to follow up an MQL within a fixed window (24 to 48 hours is the common default; a lead's engagement decays the way [the welcome window](/principles/the-welcome-window.md) describes). It sets up a feedback loop where sales marks why rejected MQLs were rejected, the signal that recalibrates the score. Without that loop the score drifts and sales stops trusting marketing's leads.
