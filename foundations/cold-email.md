...
Even inside the window, cold email works against the asset the rest of this programme spends its effort building. A cold list has no engagement history, leaving the provider nothing good to weigh. The early complaints and spam-trap hits that cold sending attracts then feed straight into the sender-level reputation that decides placement. That is the mechanism in [engagement is the new deliverability](/principles/engagement-is-deliverability.md): weak engagement is read as a property of the sender, which is why a cold campaign underperforms on its own and drags down the placement of the warmed, consented programme sharing that reputation. The complaint thresholds the bulk-sender rules enforce are easy to breach with a list that never opted in.

In practice, if cold outreach is run at all, it is run on separate sending infrastructure (a distinct domain and IP) so it cannot damage the warmed estate, at low volume, to genuinely relevant recipients, with the opt out honoured instantly. Those are damage-limitation guardrails, not a recipe for it working well. The lawful window is real but narrow. Inside it, cold email still earns less and risks more than the consent-first path it bypasses.
