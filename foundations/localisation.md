...
Non-Latin scripts reach into the channel mechanics. On [SMS](/channels/sms-and-rcs.md) any character outside the `GSM-7` set forces the whole message into `UCS-2`, which cuts the per-segment limit from 160 characters to 70 and can make a localised SMS cost several times its English equivalent for the same words. Right-to-left scripts such as Arabic and Hebrew need the layout direction set, not just the text swapped. [Accessibility](/foundations/message-design-and-rendering.md) depends on declaring the message language so a screen reader pronounces it correctly. In a global programme these details decide whether a message renders or arrives garbled.
