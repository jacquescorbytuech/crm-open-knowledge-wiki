...
Treating a user's weekly attention as an allowance that every channel draws down assumes the cap is there to be spent. It is not. The cap is a safety rail. The discipline that governs volume is to hold one cross-channel ceiling and then judge each send on its own merit, which leaves most users well below it.
...
A programme fails in two directions here. Let each channel fill its own quota and the customer is over contacted; treat the shared cap as a quota to fill in turn and you have moved the same mistake up a level. The ceiling bounds the harm, but the per message value test is what sets volume: it floats below the cap whenever another send would not earn its place.

Be clear about what that value test actually costs. Deciding per person and per message whether contact raises the probability of the action you want is uplift modelling, also called incremental modelling: estimating the causal effect of a send against the counterfactual where it never went out. It needs [holdouts](/measurement/holdouts-and-control-groups.md) feeding the model, enough volume to estimate effects per segment and tolerance for a system that withholds messages it cannot justify. Most programmes do not have this. A rules engine that fires on triggers is not it. The workable version is to enforce the hard cap, which you can always do, then approximate value with the proxies you have, recency, prior response, a propensity score, while treating true per send uplift as the harder system the capable platforms are reaching for, not a setting you switch on.
...
Default to the cheapest channel that does the job, reserving the interruptive, expensive ones for what only they can do.

## Why capping matters

A frequency cap is a hard ceiling on messages per channel and in total over a rolling window, applied before send. Caps protect the two things over mailing destroys. Deliverability suffers because complaint and disengagement signals drive placement, sliding a fatigued list toward the spam folder. The long run value of the list suffers because a fatigued subscriber unsubscribes once and is gone. Frequency is not a minor dial: too many messages is consistently the single most common reason people unsubscribe. See [respect the recipient](/principles/respect-the-recipient.md) and [segmentation has costs](/principles/segmentation-has-costs.md).
...
Apply the lowest cap a user qualifies for, treating SMS as scarcer than email at every tier because it is interruptive and costed per send.
...
3. Then narrow. Pull the cap back where the fatigue signals turned, then confirm that revenue holds while complaints fall.
...
* Hard bounces: the address or number is dead. Never send to it again.
...
* Quiet hours: do not send outside the channel's permitted window. US SMS quiet hours are commonly observed as 8am to 9pm in the recipient's local time under TCPA practice.
* Frequency-capped: the user has hit their cap for this window; defer or drop by priority.
* Recent contact and exclusions: recent purchasers, open support tickets and any campaign exclusion list.

Consent based suppressions are legal constraints everywhere; quiet hours are statutory in some jurisdictions and expected practice in the rest. See [consent and preferences](/foundations/consent-and-preferences.md).
