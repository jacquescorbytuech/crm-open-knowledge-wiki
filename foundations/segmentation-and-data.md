...
When writing expressions, be explicit about time windows (last 30 days, not recently), because a vague window becomes a different segment on every platform. Write the consent and suppression conditions into the rule itself too, so that a segment can never resolve to a contact you may not message.

## Segment overlap and the cascade into frequency

Confirm whether segments overlap before combining or sending them together. Free users and users who have not completed onboarding are not mutually exclusive; adding them double-counts the audience. Worse, a contact who sits in several concurrently scheduled segments receives several sends in the same window. Overlap is a frequency problem as much as a counting error: it is one of the main ways a programme over-contacts a subset of its list without any single campaign looking heavy.
...
Redrawing every segment to be mutually exclusive is rarely possible. Let a single contact strategy arbitrate overlap at send time instead, enforcing the cap across all segments at once rather than within each one.
