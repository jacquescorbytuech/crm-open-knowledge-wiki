...
* **Key retail and seasonal moments.** The fixed anchors the plan bends around: your category's peak periods, paydays, the public holidays of your main markets and your own launch and renewal dates. Place these first, then plan campaigns relative to them.
...
4. **Build.** CRM specialist assembles the message in the platform; wires links and UTMs; sets the suppression and frequency logic.
...
* [ ] **Links** all resolve to the right pages with no placeholders; **UTMs** present and consistent.
...
* [ ] **SMS** sender identity correct; the message within the intended segment count (`GSM-7` versus `UCS-2`); the single link resolving; `STOP` and `HELP` handling live; the send inside quiet hours.
* [ ] **Push** title and body within the lock-screen truncation; the deep link opening the right screen; the payload rendering on both iOS and Android; notification caps respected.
* [ ] **In-app** rendering at the device sizes you support; firing on the intended trigger and audience; dismissing cleanly without trapping the session.
