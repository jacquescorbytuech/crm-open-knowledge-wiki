...
tags: [transactional, marketing, consent, deliverability, sending-streams, can-spam, pecr, orchestration]
generated:
...
> Pad an order confirmation with enough marketing and regulators judge it a commercial message: it now needs marketing consent and an unsubscribe path, while dragging the transactional stream's reputation toward the marketing one's.

Because the categories are defined by primary purpose, the legal classification can flip the moment a transactional message is padded with promotion. Under the US CAN-SPAM rule a message is judged on its primary purpose: a "transactional or relationship" message dressed up with enough marketing content becomes a commercial message, subject to the unsubscribe and identification requirements that transactional mail is otherwise exempt from. The practical failure is the order confirmation stuffed with recommended products and a sale banner: it now needs marketing consent and an unsubscribe path, while dragging the transactional stream's reputation toward the marketing stream's. Other regimes draw the same boundary in their own terms: the ICO treats a routine service message as direct marketing once it carries significant promotional content, which puts it back under PECR's consent or soft opt-in requirement.

The reverse error is just as common: labelling a genuine marketing send as "transactional" (a "your account" email that is really a campaign) to dodge consent and unsubscribe rules. Regulators and inbox providers both judge on content and intent rather than the label you apply, which is why the relabelling does not work and risks a complaint-driven reputation hit.
...
* **Classify by primary purpose, once, at design time.** Decide whether a template is transactional or marketing before it ships, then route it to the matching stream. An ambiguous template is a template to split.
...
* **Track consent state correctly.** A marketing unsubscribe must not stop a transactional message; a transactional relationship must not be treated as marketing consent. Store the basis per message category, not per contact.
