...
- **House style.** Plain Markdown; prose over bullet-soup where a paragraph reads better; no em-dashes in concept files (use commas or restructure).
...
Keep titles short and in sentence case. Inside the box, follow the bundle's house style: no em-dashes and no invented facts. Do not box tables, the **Related** list or footnote definitions. A callout should wrap text that already reads as prose elsewhere on the page, not introduce new claims.
