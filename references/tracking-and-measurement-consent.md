...
Article 5(3) applying does not remove the GDPR; both govern the same measurement. Recording who opened or clicked is processing of personal data, so it always needs a lawful basis under Article 6 and must meet the GDPR's transparency, purpose-limitation and data-subject-rights obligations. Where Article 5(3) requires consent for the access to the device, that consent is also the operative GDPR basis for the collection: the EDPB's **Opinion 5/2019** on the interplay between the two instruments treats Article 5(3) as the more specific rule, so you cannot place the pixel on consent and then reclassify the same collection as legitimate interest to avoid the consent requirement. The reverse case matters just as much. Click tracking usually works by rewriting the link through a redirect that logs the click on the server before forwarding, which stores and reads nothing on the recipient's device, so Article 5(3) is not necessarily engaged. The GDPR still is, because the click tied to a person is personal data. Every open and click therefore raises a GDPR question; the open pixel raises an ePrivacy one as well.
...
The email pixel is one instance of a rule that is not about email. Two questions decide the position on any channel, and it is worth answering both rather than carrying the email pixel's answer across. First, is the measurement processing personal data? Anything tied to an identifiable person is, so the GDPR applies, which is why the permission to reach someone and the permission to measure them are separate grants on every channel, device or not. Second, does the measurement store information on, or read information already on, the recipient's device? EDPB Guidelines 2/2023 read Article 5(3) as technology-neutral, and the terminal equipment it protects is the smartphone, laptop or connected TV, not the inbox. Where that second answer is yes, ePrivacy requires consent for the device access as well as the GDPR obligations; where it is no, only the GDPR applies. The channels differ mainly in how often the second answer is yes.
