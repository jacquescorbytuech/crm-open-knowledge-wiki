...
The proxy trap is not email's alone. A push delivery receipt confirms the OS accepted the payload, not that anyone saw it; an SMS carrier `DLR` is advisory and routinely lies; an in-app impression counts a render, not attention. Every channel hands you a cheap surface metric that stands in for a behaviour you cannot see, and the same discipline applies: treat it as direction, measure the outcome. Email's proxy is the most studied and the most distorted of these.
