...
description: How to read the effect of a platform change (a new spam policy, a summarisation feature, a tab re-sort) on your own sends without platform cooperation; the limits of each method.
...
When a platform changes how it filters, ranks or summarises what you send (the dated changes in [platform interventions](/references/platform-interventions.md)), it does not report what it did to your sends. These methods estimate the effect from your own data. They are channel-agnostic in shape and channel-specific in detail, though none reads the wording layer, what a summary kept versus what you wrote, directly.

Before reaching for any of them: most senders should not attempt to read intermediation effects at all. The methods need volume far above typical thresholds, hundreds of thousands of sends per cell and often millions. Below that floor they return wide intervals around zero whatever the platform actually did. None of this applies to you unless you operate at platform scale. Below that floor the useful output is the diagnostic, not the measurement: since intermediation is bending your numbers by an amount you cannot cleanly say, treat them with that suspicion instead of spending effort quantifying the distortion.
